...
title: "Owen Editor Plugin Design"
status: adopted
product: "owen-editor"
surface: "Obsidian floating toolbar, palette and settings"
kind: plugin
updated: 2026-08-12
...
# Owen Editor Plugin Design

## Decision Summary

| 항목 | 결정 |
| --- | --- |
| 사용자 | Obsidian에서 Markdown 보고서와 기술 문서를 반복 편집하는 작성자 |
| Output job | 문맥을 벗어나지 않고 Markdown formatting·table·callout·code title 작업을 수행한다 |
| Primary object | 현재 selection과 Markdown command |
| Primary action | floating toolbar 또는 palette에서 command를 실행한다 |
| Composition | Compact Clear Glass toolbar + opaque command palette + SettingsPanel |
| 핵심 판단 | Editor content를 가리지 않는 compact command layer를 유지하고 Obsidian editor surface는 host가 소유한다 |

## Context

Owen Editor는 Obsidian 위에 format command와 persistent code title을 제공하는 plugin이다. Toolbar position, scale, density, collapse와 en/ko command vocabulary를 관리한다.
...
- main.ts
- styles.css
- code-block-titles.ts
- frosted-scrollbars.ts
- i18n.ts
...
- Clear Glass toolbar fill·rim·backdrop·shadow
- Command palette body와 repeated rows는 opaque
- en/ko typed i18n catalog
- Mobile compact toolbar와 persisted density

### Excluded Or App-Owned

- Obsidian core editor와 modal DOM 재구현
- Toolbar에 모든 command를 동시에 노출
- 앱 전역 CSS selector

## Design Decisions

### D-01. Toolbar는 bounded command layer다

- **결정**: 자주 쓰는 command만 2행 grid에 두고 나머지는 palette로 이동한다
- **이유**: Editor content와 selection을 가리지 않아야 한다
- **검증**: Viewport 375px과 760px 전후에서 overlap과 target size를 확인한다

### D-02. Context toolbar는 selection에 종속된다

- **결정**: Table·code·selection이 있을 때만 가까운 위치에 나타나고 dismiss 후 editor focus를 복원한다
- **이유**: 항상 보이는 두 번째 toolbar는 명령 중복과 시각 소음을 만든다
- **검증**: Selection change, Escape, focus restore를 plugin smoke test한다

### D-03. Command vocabulary는 bilingual이다

- **결정**: Setting, notice, error와 command label을 en/ko catalog에서 제공한다
- **이유**: 문서 언어와 host locale이 달라도 기능 의미를 유지해야 한다
- **검증**: i18n key parity와 fallback test를 release gate로 실행한다
...
- npm typecheck and test
- eslint-plugin-obsidianmd
- en/ko i18n parity and mobile toolbar review
